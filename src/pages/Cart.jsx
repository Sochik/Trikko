...
import { bag_cream } from '../assets/images'
...
export default function Cart() {
  ...
  return (
    <main className="relative before:absolute before:h-60 before:w-2/3 before:bg-primary before:-top-32 before:-right-72 before:-rotate-45 before:rounded-full py-20 min-h-[70vh]">
      <section className="container mx-auto"> <div className=" bg-white shad rounded-md p-4 max-w-screen-md mx-auto relative">
        <div className="flex items-center gap-4 justify-between">
          <h3 className="text-2xl md:text-4xl font-bold text-primary">Trikko <span className="text-secondary"> Cart</span></h3>
          <div className="h-8 w-8 text-secondary hover:text-dark cursor-pointer flex justify-center items-center text-lg relative md:scale-125 -translate-x-1"><HiOutlineShoppingBag />
        ...
        <div className="flex flex-col py-4 divide-y divide-slate-200">
          {
            cartContents.map((el, i)=> (
              <aside key={el.id} className="p-2 flex items-center gap-2 justify-between relative">
                <button onClick={() => dispatch(removeFromCart(i))} className="h-5 w-5 text-lg grid place-items-center bg-slate-100 text-primary cursor-pointer absolute -top-2 -right-1 rounded-full">
                  <IoCloseOutline />
                </button>
                ...
                <div className="border border-slate-200 rounded-md w-10 h-8 overflow-hidden">
                  <input onChange={e => dispatch(changeInCart({id: el.id, qty: e.target.value}))} type="number" min={1} value={el.qty} className="bg-transparent w-16 px-2 outline-none pt-0.5 pl-2.5 text-sm md:text-base" />
                </div>
              </aside>
            ))
          }
